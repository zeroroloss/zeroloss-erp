...
- 본사와 지점 계정을 구분하여 로그인합니다.
- 권한에 따라 접근 가능한 메뉴와 홈 화면이 달라집니다.
- 지점 정보와 키오스크 정보가 함께 연동됩니다.

## 2️⃣ 본사 계정 기능

본사 계정은 여러 지점의 운영 상태를 한 번에 확인하고, 승인과 조정이 필요한 업무를 처리하는 역할입니다.

- 지점 관리
- 지점 검색 및 상세 확인
- 지점별 상태 점검 및 운영 현황 확인
- 발주 요청 승인/반려
- 승인 수량 조정 후 출고 처리
- 출고 헤더 및 출고 상세 생성
- 창고 재고 차감 및 재고 이력 관리
...
지점에서 올라온 발주 요청은 본사 계정에서 확인한 뒤, 승인 또는 반려로 처리됩니다. 승인 시에는 승인 수량이 기준이 되고, 출고 및 재고 차감까지 이어져 본사 재고 흐름이 일관되게 관리됩니다.

## 3️⃣ 지점 계정 기능

지점 계정은 매장 운영에 필요한 요청과 현장 처리를 중심으로 동작합니다.

- 지점 메인 화면에서 오늘의 운영 상태 확인
- 입고 등록 및 입고 내역 확인
- 지점 재고 조회 및 재고 변동 확인
- 발주 요청 등록 및 요청 내역 확인
- 레시피 조회를 통한 자재 기준 확인
- 매출 조회 및 일자별 운영 확인
- 재고 조정 및 교환 처리
- 고객지원 문의 등록 및 처리 결과 확인
- 현장 운영에 필요한 메뉴와 공통 안내 확인

지점 계정에서는 직접 승인권을 갖기보다 필요한 자재와 물품을 요청하는 역할이 중심입니다. 입고와 재고 변동, 발주 요청, 교환, 고객지원처럼 매장 운영에서 즉시 필요한 업무를 처리하도록 구성했습니다.

## 4️⃣ 발주 및 출고 처리

- 지점의 발주 요청을 본사에서 확인합니다.
- 승인 시 승인 수량 기준으로 출고 헤더와 출고 상세가 생성됩니다.
- 창고 재고를 FIFO 방식으로 차감하고, 재고 이력을 남깁니다.
- 반려 사유를 기록하여 지점에 다시 전달합니다.

## 5️⃣ 키오스크 연동

- 키오스크 주문, 주문 유형, 결제 및 주문 상태 데이터를 ERP에서 관리합니다.
- 지점별 키오스크 배너 및 주문 흐름을 함께 운영할 수 있습니다.
- 주문 데이터가 재고 및 운영 관리와 연결되도록 설계했습니다.
...
| 경로 | 설명 |
|---|---|
| [build.gradle](build.gradle) | Gradle 빌드 설정 파일입니다. 의존성, 컴파일 설정, 패키징 방식을 관리합니다. |
...
1. 저장소를 클론합니다.
2. `build.gradle` 기준으로 프로젝트를 빌드합니다.
3. Tomcat에 배포한 뒤 웹 애플리케이션을 실행합니다.
4. MariaDB 연결 정보와 MyBatis 설정을 환경에 맞게 조정합니다.
