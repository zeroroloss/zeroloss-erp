...
<script>
    var categoryMaterialMap = <%=request.getAttribute("categoryMaterialJson") == null ? "{}" : request.getAttribute("categoryMaterialJson")%>;
</script>
</head>
    ...
    <main class="p-6">
        ...
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
            <div class="px-6 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                ...
                <p class="text-base text-gray-500">조회 건수 <span id="totalRecords" class="font-semibold text-gray-700">0건</span></p>
            </div>
    ...
    <div class="bg-white rounded-lg max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div>
                ...
                <select id="formCategory" onchange="updateFormItemNames()"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#00853D] focus:border-transparent">
                    <option value="">선택하세요</option>
                    <%
                    if (categoryMaterialMap != null) {
            ...
            <div>
                ...
                <input type="number" id="formQuantity" min="1" placeholder="수량 입력"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#00853D] focus:border-transparent">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">단가 <span class="text-red-600">*</span></label>
                <input type="number" id="formUnitPrice" min="0" placeholder="단가 입력"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#00853D] focus:border-transparent">
            </div>
            <div>
        ...
        <div class="flex justify-end gap-2">
            ...
            <button type="button" onclick="handleReceive()"
                class="px-4 py-2 bg-[#00853D] text-white rounded-lg hover:bg-[#006B2F] font-medium transition-colors">입고 등록</button>
        </div>
    </div>
</div>
...
<script>
    ...
    document.getElementById('formUnitPrice').addEventListener('input', calculateTotal);

    function calculateTotal() {
        var quantity  = parseInt(document.getElementById('formQuantity').value) || 0;
        var unitPrice = parseInt(document.getElementById('formUnitPrice').value) || 0;
        document.getElementById('totalAmount').textContent = (quantity * unitPrice).toLocaleString('ko-KR');
    }
    ...
    async function handleReceive() {
        var supplier   = document.getElementById('formSupplier').value;
        var category   = document.getElementById('formCategory').value;
        var itemName   = document.getElementById('formItem').value;
        var quantity   = parseInt(document.getElementById('formQuantity').value) || 0;
        var unitPrice  = parseInt(document.getElementById('formUnitPrice').value) || 0;
        var expiryDate = document.getElementById('formExpiryDate').value;

        if (!supplier || !category || !itemName || !quantity || !unitPrice || !expiryDate) {
        ...
        try {
            var res = await fetch('<%=request.getContextPath()%>/api/hq/warehouse/inbound', {
                method: 'POST',
            ...
            if (!result || result.status != 'success') throw new Error((result && result.message) || '등록 실패');
    ...
    function renderTable() {
        ...
        if (pageItems.length == 0) {
            tbody.innerHTML =
                '<tr><td colspan="9" class="py-12 text-center text-gray-500">' +
        ...
        pageItems.forEach(function(record) {
            var unitPriceFmt = record.unitPrice.toLocaleString('ko-KR');
            var totalFmt     = record.totalPrice.toLocaleString('ko-KR');
            var tr = document.createElement('tr');
            tr.className = 'border-b border-gray-100 hover:bg-gray-50';
            tr.innerHTML =
                '<td class="py-4 px-6 text-sm text-gray-900">'                   + record.inboundAt  + '</td>' +
                '<td class="py-4 px-6 text-sm text-gray-900">'                   + record.supplier   + '</td>' +
                '<td class="py-4 px-6 text-sm text-gray-600">'                   + record.category   + '</td>' +
                '<td class="py-4 px-6 text-sm font-medium text-gray-900">'       + record.itemName   + '</td>' +
                '<td class="py-4 px-6 text-right text-sm text-gray-600">'        + record.quantity + ' ' + (record.unit || '') + '</td>' +
                '<td class="py-4 px-6 text-right text-sm text-gray-600">&#8361;' + unitPriceFmt + '</td>' +
                '<td class="py-4 px-6 text-right text-sm font-semibold text-[#00853D]">&#8361;' + totalFmt + '</td>' +
                '<td class="py-4 px-6 text-sm text-gray-600">'                   + record.expiryDate + '</td>' +
                '<td class="py-4 px-6 text-sm text-gray-600">'                   + (record.handler || '-') + '</td>';
    ...
    function updatePagination(totalPages, startIndex, endIndex) {
        ...
        for (var page = 1; page <= totalPages; page++) {
            ...
            if (showPage) {
                ...
                (function(p) { btn.onclick = function() { goToPage(p); }; })(page);
