...
<%
    ...
    boolean branchInventoryActive = uri.endsWith("/hq/branch_stock/branch-inventory-status.jsp");
    boolean lossManagementActive = uri.endsWith("/hq/branch_stock/loss-management.jsp");
    ...
    boolean orderDashboardActive = uri.endsWith("/hq/place_order/order-requests-dashboard.jsp");
    boolean orderManagementActive = uri.endsWith("/hq/place_order/order-requests-management.jsp");
    boolean orderLimitsActive = uri.endsWith("/hq/place_order/order-item-limits.jsp");
    ...
    boolean hqInquiriesActive = uri.endsWith("/hq/support/headquarters-inquiries.jsp");
...
<nav class="flex-1 p-4 space-y-1 overflow-y-auto">
    ...
    <button onclick="toggleMenu(this)" class="w-full flex items-center justify-between px-3 py-2.5 rounded-lg <%= supportGroup ? "bg-gray-100 text-gray-700" : "text-gray-700 hover:bg-gray-100" %> transition-colors">
        <div class="flex items-center gap-3">
            ...
            <span class="text-sm">직영점 관리</span>
        </div>
    ...
    <div class="submenu <%= supportGroup ? "" : "hidden" %> ml-4 mt-1 space-y-1">
        <a href="<%= cp %>/hq/support/branch-search.jsp" class="block px-4 py-2 rounded-lg text-sm <%= branchSearchActive ? "bg-[#00853D] text-white font-medium" : "text-gray-600 hover:bg-gray-100" %>">직영점 조회</a>
        ...
        <a href="<%= cp %>/hq/support/headquarters-inquiries.jsp" class="block px-4 py-2 rounded-lg text-sm <%= hqInquiriesActive ? "bg-[#00853D] text-white font-medium" : "text-gray-600 hover:bg-gray-100" %>">문의 게시판</a>
    </div>
