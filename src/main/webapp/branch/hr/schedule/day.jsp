...
<head>
  ...
  <style>
    ...
    .day-cell { min-height: 170px; padding: 12px; position: relative; }
    ...
    @media (max-width: 1200px) { .title { font-size: 30px; } .sub { font-size: 16px; } .add-btn { font-size: 14px; height: 42px; } .k { font-size: 13px; } .v { font-size: 24px; } .panel-head { font-size: 16px; } .field label { font-size: 14px; } .date-label { font-size: 22px; } .input, .today, .day-head, .legend, .view-tabs a { font-size: 14px; } .stats { grid-template-columns: repeat(2, 1fr); } }
    @media (max-width: 760px) { .title { font-size: 30px; } .sub { font-size: 14px; } .filter-grid { grid-template-columns: 1fr; } .stats { grid-template-columns: 1fr; } .date-label { font-size: 22px; } .legend { font-size: 13px; } .add-btn { font-size: 14px; } .input { font-size: 14px; } }
  </style>
    ...
    <main class="zl-page">
      <div class="wrap">
        <div class="head">
          ...
          <a class="add-btn" href="<%= request.getContextPath() %>/branch/hr/schedule/create_schedule.jsp"><img class="ico-16" src="<%= request.getContextPath() %>/branch/icons/hr/plus.svg" alt="추가" />일정 추가</a>
        </div>
        ...
        <section class="panel">
          ...
          <div class="filter-grid"><div class="field"><label>매장</label><input class="input" type="text" /></div><div class="field"><label>직원</label><input class="input" type="text" /></div><div class="field"><label>근무 유형</label><input class="input" type="text" /></div></div>
        </section>
        <section class="cal-wrap">
          <div class="cal-top">
            ...
            <div class="view-tabs"><a href="<%= request.getContextPath() %>/branch/hr/schedule/main.jsp">월</a><a href="<%= request.getContextPath() %>/branch/hr/schedule/week.jsp">주</a><a class="active" href="<%= request.getContextPath() %>/branch/hr/schedule/day.jsp">일</a></div>
          </div>
          ...
          <div class="day-board"><div class="day-head">금요일</div><div class="day-cell"><span class="picked">17</span></div></div>
        </section>
      </div>
    </main>
  </div>
