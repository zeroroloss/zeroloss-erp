...
<head>
    ...
    <title>매출 조회 - 시간대별</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;700;800&display=swap" rel="stylesheet">
    <style>
        :root { --green:#00853d; --line:#d1d5db; --bg:#f3f4f6; --text:#111827; --yellow:#f7cc00; }
    ...
    width: 100%;max-width: none;margin: 0;padding:28px 24px 36px;}
        ...
        .bars{position:absolute;left:18px;right:18px;bottom:0;top:0;display:grid;grid-template-columns:repeat(5,1fr);gap:22px;align-items:end;}
        .bar{background:var(--yellow);position:relative;}
        .x-label{position:absolute;bottom:-30px;left:50%;transform:translateX(-50%);font-size:12px;color:#6b7280;white-space:nowrap;}
        .legend{text-align:center;margin-top:26px;color:#f1b600;font-size:18px;font-weight:700;}
        .legend::before{content:"";display:inline-block;width:16px;height:16px;background:#f7cc00;margin-right:8px;vertical-align:middle;}
        ...
        @media (max-width:1280px){.head h1{font-size:34px}.head p{font-size:24px}.tab{font-size:14px}.input{font-size:14px}.chart-title,.result-head{font-size:28px}th,td{font-size:16px}.legend{font-size:18px}.y-label{font-size:14px}.x-label{font-size:12px}.pager{font-size:14px}.p{width:34px;height:34px;font-size:14px}}
    </style>
...
<div class="wrap">
    ...
    <section class="card">
        <div class="toolbar">
            ...
            <a class="tab active" href="/branch/sales/branch_sales/sales_hourly.jsp">시간대별</a>
            <a class="tab" href="/branch/sales/branch_sales/sales_menu.jsp">메뉴별</a>
            <div class="divider"></div>
            <select id="timeSlot" class="input">
                <option>10:00-11:00</option><option>11:00-12:00</option><option selected>12:00-13:00</option><option>13:00-14:00</option><option>14:00-15:00</option>
            </select>
    ...
    <section class="section">
        <div class="chart-wrap">
            <div class="chart-title">시간대별 매출 비교</div>
        ...
        <div class="result-card">
            ...
            <table>
                <thead><tr><th>번호</th><th>날짜</th><th>시간대</th><th>매출액</th><th>주문수</th><th>평균 객단가</th></tr></thead>
...
(function () {
    var itemsPerPage = 10;
    var currentPage = 1;
    var timeInput = document.getElementById('timeSlot');
    ...
    function seed(s){ return s.split('').reduce(function(a,c){ return (a*29 + c.charCodeAt(0)) % 7919; },19); }

    function hourlyData(slot){
        var s = seed(slot);
        var labels = ['10:00-11:00','11:00-12:00','12:00-13:00','13:00-14:00','14:00-15:00'];
        return labels.map(function(label, i){ return { label:label, sales: 300000 + ((s + i*89) % 650000) }; });
    }

    function buildResults(slot){
        var s = seed(slot);
        return Array.from({length:23}, function(_, i){
            var day = String(i + 1).padStart(2,'0');
            var hour = String(10 + (i % 10)).padStart(2,'0') + ':00';
            var sales = 2000000 + ((s + i * 61) % 500000);
            var orders = 100 + ((s + i * 7) % 50);
            return { no:i+1, date:'2026-04-' + day, time:hour, sales:sales, orders:orders, avg:Math.floor(sales/orders) };
        });
    }

    function renderChart(data){
        chartArea.innerHTML = '';
        var ticks = [1000000, 750000, 500000, 250000, 0];
        ticks.forEach(function(t){
            var y = (1 - t / 1000000) * 100;
            var line = document.createElement('div'); line.className='grid-line'; line.style.top=y + '%'; chartArea.appendChild(line);
            var yl = document.createElement('div'); yl.className='y-label'; yl.style.top=y + '%'; yl.textContent = Math.round(t/1000) + 'K'; chartArea.appendChild(yl);
        });
        var bars = document.createElement('div'); bars.className = 'bars';
        data.forEach(function(d){
            var b = document.createElement('div'); b.className='bar'; b.style.height = Math.round((d.sales/1000000)*100) + '%';
            var x = document.createElement('div'); x.className='x-label'; x.textContent=d.label; b.appendChild(x);
            bars.appendChild(b);
        });
    ...
    function renderTable(all){
        ...
        resultBody.innerHTML = rows.map(function(r){
            return '<tr><td>' + r.no + '</td><td>' + r.date + '</td><td>' + r.time + '</td><td>' + won(r.sales) + '</td><td>' + r.orders + '건</td><td>' + won(r.avg) + '</td></tr>';
        }).join('');
    ...
    function render(){
        var slot = timeInput.value;
        renderChart(hourlyData(slot));
        renderTable(buildResults(slot));
    }

    document.getElementById('searchBtn').addEventListener('click', function(){ currentPage=1; render(); });
    document.getElementById('resetBtn').addEventListener('click', function(){ timeInput.value='12:00-13:00'; currentPage=1; render(); });
    render();
})();
</script>
</body>
