...
<head>
    ...
    width: 100%;max-width: none;margin: 0;padding:28px 24px 36px;}
        ...
        .v-grid{position:absolute;top:0;bottom:0;border-left:1px dashed #d1d5db;}
        ...
        .legend::before{content:"";display:inline-block;width:16px;height:4px;background:#00853d;margin-right:8px;vertical-align:middle;}
...
<div class="wrap">
    ...
    <section class="card">
        <div class="toolbar">
            ...
            <a class="tab active" href="/branch/sales/branch_sales/sales_period.jsp">기간별</a>
            ...
            <input id="startDate" class="input" type="date" value="2026-03-26">
            <input id="endDate" class="input" type="date" value="2026-04-02">
...
(function () {
    ...
    function seed(s){ return s.replace(/-/g,'').split('').reduce(function(a,c){ return (a*31 + Number(c)) % 9973; },13); }

    function periodData(startDate, endDate){
        var s = seed(startDate + endDate);
        var labels = ['3/26','3/27','3/28','3/29','3/30','3/31','4/1','4/2'];
        return labels.map(function(label, i){
            return { label: label, sales: 2500000 + ((s + i*79) % 130) * 10000 };
        });
    }

    function buildResults(startDate, endDate){
        var s = seed(startDate + endDate);
        return Array.from({length:23}, function(_, i){
            var day = String(i + 1).padStart(2,'0');
            var sales = 2000000 + ((s + i * 67) % 500000);
            var orders = 100 + ((s + i * 11) % 50);
            return { no:i+1, date:'2026-04-' + day, sales:sales, orders:orders, avg:Math.floor(sales/orders) };
        });
    }
    ...
    function renderChart(data){
        ...
        ticks.forEach(function(t){
            ...
            var line = document.createElement('div'); line.className = 'grid-line'; line.style.top = y + '%';
            var label = document.createElement('div'); label.className = 'y-label'; label.style.top = y + '%'; label.textContent = (t / 1000000).toFixed(1) + 'M';
            chartArea.appendChild(line); chartArea.appendChild(label);
        });

        data.forEach(function(d, i){
            var x = (i / (data.length - 1)) * 100;
            var v = document.createElement('div'); v.className = 'v-grid'; v.style.left = x + '%'; chartArea.appendChild(v);
            var xl = document.createElement('div'); xl.className = 'x-label'; xl.style.left = x + '%'; xl.textContent = d.label; chartArea.appendChild(xl);
        });
        ...
        var points = data.map(function(d, i){
            var x = (i / (data.length - 1)) * chartArea.clientWidth;
            var y = (1 - d.sales / 3800000) * chartArea.clientHeight;
            return {x:x, y:y};
        });

        var poly = document.createElementNS('http://www.w3.org/2000/svg', 'polyline');
        poly.setAttribute('fill', 'none');
        poly.setAttribute('stroke', '#00853d');
        ...
        points.forEach(function(p){
            ...
            dot.setAttribute('cx', p.x); dot.setAttribute('cy', p.y); dot.setAttribute('r', '4'); dot.setAttribute('fill', '#fff'); dot.setAttribute('stroke', '#00853d'); dot.setAttribute('stroke-width', '2');
            svg.appendChild(dot);
        });
    ...
    function renderTable(all){
        ...
        resultBody.innerHTML = rows.map(function(r){
            return '<tr><td>' + r.no + '</td><td>' + r.date + '</td><td>' + won(r.sales) + '</td><td>' + r.orders + '건</td><td>' + won(r.avg) + '</td></tr>';
        }).join('');
    ...
    function render(){
        var s = startInput.value; var e = endInput.value;
        renderChart(periodData(s,e));
        renderTable(buildResults(s,e));
    }

    document.getElementById('searchBtn').addEventListener('click', function(){ currentPage = 1; render(); });
    document.getElementById('resetBtn').addEventListener('click', function(){ startInput.value='2026-03-26'; endInput.value='2026-04-02'; currentPage=1; render(); });
