        ...
        <div class="filters">
            <div class="filter-group">
                <span class="label">조회기간:</span>
                    ...
                    <input type="text" id="period-start" placeholder="시작일" readonly onclick="openCustomDatePicker('period-start', event)">
                </div>
                <span class="text-gray-500">~</span>
                <div class="date-picker-wrap">
                    <input type="text" id="period-end" placeholder="종료일" readonly onclick="openCustomDatePicker('period-end', event)">
                </div>
            </div>
            <div class="filter-group">
...
<script>
    ...
    // initialize defaults and custom picker behavior
    ...
    if (!document.getElementById('period-start').value) document.getElementById('period-start').value = thirtyDaysAgoStr;
    ...
    function parseDateLocal(dateStr) {
        if (!dateStr) return null; var parts = String(dateStr).split('-'); if (parts.length !== 3) return null;
    ...
    window.openCustomDatePicker = function(inputId, event) {
        if (event) event.stopPropagation(); customDateTargetId = inputId; var input = document.getElementById(inputId);
        var selected = parseDateLocal(input.value); customPickerDate = selected || new Date(); renderCustomDatePicker(); positionCustomDatePicker(input);
        document.getElementById('customDatePicker').classList.remove('hidden');
    };

    function positionCustomDatePicker(input) {
        var picker = document.getElementById('customDatePicker'); var rect = input.getBoundingClientRect(); var pickerWidth = 300; var pickerHeight = 330;
    ...
    window.selectCustomDate = function(dateValue) {
        ...
        if (customDateTargetId === 'period-start') { var sd = parseDateLocal(dateValue); var edVal = document.getElementById('period-end').value; var ed = parseDateLocal(edVal); if (ed && ed < sd) document.getElementById('period-end').value = dateValue; }
        else if (customDateTargetId === 'period-end') { var ed = parseDateLocal(dateValue); var sdVal = document.getElementById('period-start').value; var sd = parseDateLocal(sdVal); if (sd && sd > ed) document.getElementById('period-start').value = dateValue; }
