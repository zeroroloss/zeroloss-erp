...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;}body{margin:0;font-family:"Noto Sans KR","Malgun Gothic",sans-serif;background:var(--bg);color:var(--text);} .wrap{
    ...
    table{width:100%;border-collapse:collapse;} th,td{padding:12px 16px;border-bottom:1px solid #e5e7eb;font-size:14px;} th{background:#f9fafb;text-align:left;color:#374151;font-weight:700;} td:nth-child(4),th:nth-child(4),td:nth-child(6),th:nth-child(6),td:nth-child(7),th:nth-child(7){text-align:center;}
    .badge-state{display:inline-block;padding:4px 10px;border-radius:999px;font-size:12px;font-weight:700;} .pending{background:#fef3c7;color:#b45309;} .done{background:#dcfce7;color:#15803d;}
    .btn{border:none;border-radius:8px;padding:7px 12px;color:#fff;font-size:12px;font-weight:700;cursor:pointer;} .ok{background:#16a34a;} .no{background:#dc2626;}
    @media (max-width:1280px){.title{font-size:40px}.sub{font-size:24px}.tab{font-size:16px;height:44px}.panel-title{font-size:24px}.panel-sub{font-size:16px}th,td{font-size:14px}.badge-state{font-size:12px}.btn{font-size:12px}.stats{grid-template-columns:repeat(2,1fr)}.stat .n{font-size:28px}.stat .l{font-size:13px}}
  </style>
...
<main class="zl-page"><div class="wrap">
  ...
  <nav class="tabs">
    ...
    <a class="tab active" href="<%= request.getContextPath() %>/branch/swap/received_request.jsp"><img class="tab-ico" src="<%= request.getContextPath() %>/branch/icons/swap/inbox.svg" alt="">받은 요청 <span class="badge">2</span></a>
